...
def run_exhaustive_audit():
    print("🚀 --- InvisID Randomized Multi-Identity EXHAUSTIVE Audit --- 🚀")
    ...
    # Pool of possible identities
    id_pool = ["EMP-001", "EMP-002", "EMP-003", "CON-004", "INT-005", "GST-006", "EMP-007", "EMP-008", "EMP-999", "ADMIN"]
    
    print(f"📂 Found {len(files)} assets. Starting full-suite randomized audit...\n")
    overall_results = []

    for chosen_file in files:
        target_id = random.choice(id_pool)
        print(f"\n🔍 Testing Asset: {chosen_file} | Assigned Identity: {target_id}")
        print("-" * 60)
        ...
        embed_watermark(img, target_id, output_path=temp_wm_path)
        watermarked = cv2.imread(temp_wm_path)
        
        # FULL 14-ATTACK EXHAUSTIVE SUITE
        attacks = [
            ("EXIF Sidecar Recovery", "exif"), 
            ("Baseline (Spatial)", lambda x: x),
        ...
        for name, attack in attacks:
            try:
                if name == "EXIF Sidecar Recovery":
                    extracted_id, conf, _ = extract_watermark(temp_wm_path, master_data=img)
                else:
                    attacked = attack(watermarked)
                    extracted_id, conf, _ = extract_watermark(attacked, master_data=img, ignore_exif=True)
                
                status = "✅ PASS" if extracted_id == target_id else f"❌ FAIL (Got: {extracted_id})"
